...
- `./start.sh` — sobe **somente** backend, frontend e gateway (a infraestrutura — BDs, GeoServers, jobs — deve já estar rodando após o `./setup.sh`). Não pergunta agendas de job. Se você fez `docker compose down` sem `-v`, suba a infra com o comando `docker compose` indicado quando o `./start.sh` falhar, depois rode `./start.sh` de novo.
