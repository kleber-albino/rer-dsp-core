...
`./config.sh` writes the SeaweedFS endpoint, bucket, region, credentials, path-style and the cron
to `application/application.yaml` and to `.env`.
...
```bash
docker compose --env-file .env --profile object-storage run --rm --build \
